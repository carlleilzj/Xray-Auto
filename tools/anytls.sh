...
echo -e "${GREEN}================================================${PLAIN}"
echo ""
echo -e "  ${YELLOW}anytls://${PASSWORD}@${IP}:${PORT}?insecure=1${PLAIN}"
[[ -n "$IP6" ]] && echo -e "  ${YELLOW}anytls://${PASSWORD}@[${IP6}]:${PORT}?insecure=1${PLAIN}"
echo -e ""
echo -e "${GREEN}================================================${PLAIN}"
echo -e "${GREEN}  Surge / Stash 配置（需配合本地客户端）${PLAIN}"
echo -e "${GREEN}================================================${PLAIN}"
echo -e ""
echo -e "  1. 下载客户端:"
echo -e "     ${YELLOW}curl -sLO https://github.com/anytls/anytls-go/releases/download/v0.0.13/anytls_0.0.13_darwin_arm64.zip${PLAIN}"
echo -e "     ${YELLOW}unzip anytls_*.zip && chmod +x anytls-client${PLAIN}"
echo -e ""
echo -e "  2. 启动本地 SOCKS5 代理:"
echo -e "     ${YELLOW}./anytls-client -l 127.0.0.1:1080 -s anytls://\${PASSWORD}@\${IP}:\${PORT}?insecure=1${PLAIN}"
echo -e ""
echo -e "  3. Surge 配置代理:"
echo -e "     ${GREEN}[Proxy]${PLAIN}"
echo -e "     ${GREEN}anytls = socks5, 127.0.0.1, 1080${PLAIN}"
echo -e ""
echo -e "${GREEN}================================================${PLAIN}"
